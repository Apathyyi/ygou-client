...
class MsgrecommendAdpater extends MultiRecyclerAdapter {
     ...
     * @param data
     */
    public MsgrecommendAdpater(List<MultipleitemEntity> data,AddFriendLisenter lisenter) {
        super(data);
        this.lisenter = lisenter;
        addItemType(ItemType.RECOMMEND, R.layout.item_pullmsg);
    }
    ...
    protected void convert(MulitipleViewHolder helper, MultipleitemEntity item) {
        ...
        if (tag){
            ...
            btn_botify.setText("已发送");
        }else{
            btn_botify.setText("添加");
        }
